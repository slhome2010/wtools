...
class ControllerUserUserPermission extends Controller {
    ...
    public function add() {
        ...
        $this->load->model('user/user_group');
        $response = array('user_group_id' => '');

        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $response['user_group_id'] = $this->model_user_user_group->addUserGroup($this->request->post);
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $request_post['name'] = $this->request->post['name'];
            $request_post['permission'] = [];
            $request_post_permissions = json_decode(str_replace('&quot;', '"', $this->request->post['permissions']));
            foreach ($request_post_permissions as $request) {
                //  $request_clear = str_replace('&quot;', '"', $request);
                //   $request_obj = json_decode($request_clear);
                if (isset($request->access) && $request->access == "on") {
                    $request_post['permission']["access"][] = $request->permission;
                }
                if (isset($request->modify) && $request->modify == "on") {
                    $request_post['permission']["modify"][] = $request->permission;
                }
                if (isset($request->hiden) && $request->hiden == "on") {
                    $request_post['permission']["hiden"][] = $request->permission;
                }
            }
            $this->model_user_user_group->editUserGroup($this->request->post['user_group_id'], $request_post);
            // $this->model_user_user_group->editUserGroup($this->request->get['user_group_id'], $this->request->post);
            // file_put_contents('wtools.txt', print_r($request_post, true), FILE_APPEND);
            // file_put_contents('wtools.txt', print_r(PHP_EOL, true), FILE_APPEND);
            //  $this->session->data['success'] = $this->language->get('text_success');
            //  $url = '';
            //   if (isset($this->request->get['sort'])) {
            //       $url .= '&sort=' . $this->request->get['sort'];
    ...
    public function delete() {
        ...
        if (isset($this->request->post['user_group_id']) && $this->validateDelete()) {
    ...
    public function getList() {
        ...
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($data['user_groups']));
    }
...
//
        ...
        foreach ($files as $file) {
            ...
            if (!in_array($permission, $ignore)) {
                $data['permissions'][] = array('permission' => $permission,
                    'hiden' => (isset($user_group_info['permission']['hiden']) && in_array($permission, $user_group_info['permission']['hiden'])) ? "on" : "off",
                    'access' => (isset($user_group_info['permission']['access']) && in_array($permission, $user_group_info['permission']['access'])) ? "on" : "off",
                    'modify' => (isset($user_group_info['permission']['modify']) && in_array($permission, $user_group_info['permission']['modify'])) ? "on" : "off"
                );
            }
        }
    ...
    public function getPermissions() {
        ...
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($data['permissions']));
    }

...
class ModelSaleVisaSber extends Model {
    ...
    public function getOrders($data = array()) {
		...
		(SELECT vs.customer_reference FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS customer_reference,
		(SELECT vs.transaction_status FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS transaction_status,
		(SELECT vs.card_country FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS card_country,
		(SELECT vs.card_number FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS card_number,
		(SELECT vs.verified3d FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS verified3d,
		(SELECT vs.ip_address FROM " . DB_PREFIX . "visa_sber vs WHERE vs.visa_id = o.visa_sber_id) AS ip_address,
		o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.visa_sber_id FROM `" . DB_PREFIX . "order` o";
        ...
        if (isset($data['filter_visa_id']) && !is_null($data['filter_visa_id'])) {
            $sql .= " AND o.visa_sber_id = '" . (int) $data['filter_visa_id'] . "'";
        } else {
            $sql .= " AND o.visa_sber_id > '0'";
        }
    ...
    public function editVisa($data) {
        ...
        $this->db->query("UPDATE`" . DB_PREFIX . "visa_sber` SET transaction_status = '" . $this->db->escape($ts) . "',
    ...
    public function getExtended($data) {
        ...
        if ($tranResult->return) {
            ...
            $this->db->query("UPDATE`" . DB_PREFIX . "visa_sber` SET card_country = '" . $exdata['card_country'] . "',
    ...
    public function dropDatabaseTables() {
        $sql = "DROP TABLE IF EXISTS `" . DB_PREFIX . "visa_sber`;";
        $this->db->query($sql);

        $sql = "ALTER TABLE `" . DB_PREFIX . "order` DROP COLUMN `visa_sber_id`;";
        $this->db->query($sql);
    }
